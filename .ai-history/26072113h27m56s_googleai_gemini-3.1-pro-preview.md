...
# GGL Implement special instructions for CV generation with a toggle for distinct instructions

I implement this feature by extending the existing special instructions logic to the CV generation process. I modify the `views/index.ejs` file to include a new checkbox and text area for distinct CV instructions, while updating the UI logic to allow users to use the same instructions for both the cover letter and the CV by default. I update `src/index.ts` to parse these new fields and pass them down to the API wrappers. Finally, I update the API wrappers (`src/ask-openai.ts`, `src/ask-gemini.ts`, and `src/ask-anthropic.ts`) and the prompt builder (`src/prompt.ts`) to accept and inject these instructions into the AI conversation turns for the CV generation.
  ...
  <body>
      ...
      <form class="mt-4" action="/" method="POST">
        ...
        <fieldset class="mb-3 border rounded p-3">
          ...
          <div class="mb-3 form-check">
            ...
            <label class="form-check-label" for="enableSpecialInstructions">Add custom guidance for the documents</label>
          </div>
          <div class="mb-3">
            <label for="specialInstructions" class="form-label" id="specialInstructionsLabel">Special instructions (Cover Letter & CV)</label>
          ...
          <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="separateCVInstructions" name="separateCVInstructions" value="true">
            <label class="form-check-label" for="separateCVInstructions">Use distinct instructions for the CV</label>
          </div>
          <div class="mb-0">
            <label for="cvSpecialInstructions" class="form-label" id="cvSpecialInstructionsLabel">Special instructions (CV only)</label>
            <textarea id="cvSpecialInstructions" class="form-control" rows="3" name="cvSpecialInstructions" placeholder="E.g., focus heavily on technical skills, etc."></textarea>
          </div>
        </fieldset>
        ...
        <fieldset class="mb-3 border rounded p-3">
          ...
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="providersCombination" id="comboOpenAIAnthropic" value="openai-anthropic" value="openai-anthropic" <%= selectedCombination === 'openai-anthropic' ? 'checked' : '' %>>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        const separateCVInstructionsCheckbox = document.getElementById('separateCVInstructions');
        ...
        function updateSpecialInstructionsState() {
          if (enableSpecialInstructionsCheckbox.checked) {
            ...
            separateCVInstructionsCheckbox.disabled = false;
            
            if (separateCVInstructionsCheckbox.checked) {
              specialInstructionsLabel.textContent = "Special instructions (Cover Letter)";
              cvSpecialInstructionsTextarea.disabled = false;
              cvSpecialInstructionsTextarea.placeholder = "E.g., focus heavily on technical skills, etc.";
            } else {
              specialInstructionsLabel.textContent = "Special instructions (Cover Letter & CV)";
              cvSpecialInstructionsTextarea.disabled = true;
              cvSpecialInstructionsTextarea.value = '';
              cvSpecialInstructionsTextarea.placeholder = "Distinct CV instructions disabled";
            }
          } else {
            specialInstructionsTextarea.disabled = true;
            specialInstructionsTextarea.value = '';
            separateCVInstructionsCheckbox.disabled = true;
            separateCVInstructionsCheckbox.checked = false;
            cvSpecialInstructionsTextarea.disabled = true;
            cvSpecialInstructionsTextarea.value = '';
        ...
        separateCVInstructionsCheckbox.addEventListener('change', updateSpecialInstructionsState);
...
app.post('/', csrfProtection, async (req: Request, res: Response) => {
  ...
  const separateCVInstructions = req.body.separateCVInstructions === 'true';
  const cvSpecialInstructionsInput = req.body.cvSpecialInstructions || '';

  const cvSpecialInstructions = separateCVInstructions ? cvSpecialInstructionsInput : specialInstructions;
  ...
  const geminiCVPromise = runGemini
    ? getGeminiCVResult(job, position, language, enableSpecialInstructions, cvSpecialInstructions, dryRun)
    : Promise.resolve('Not selected');

  const openAICVPromise = runOpenAI
    ? getOpenAICVResult(job, position, language, enableSpecialInstructions, cvSpecialInstructions, dryRun)
    : Promise.resolve('Not selected');

  const anthropicCVPromise = runAnthropic
    ? getAnthropicCVResult(job, position, language, enableSpecialInstructions, cvSpecialInstructions, dryRun)
    : Promise.resolve('Not selected');
...
export async function getOpenAICVResult(
  ...
  const cv = getBaseCV(language);
  const turns = getCVConversation(language, cv, jobDescription, position, enableSpecialInstructions, specialInstructions);
  const messages = [
...
GoogleAI gemini-3.1-pro-preview (15.1k in, 10.9k out)
