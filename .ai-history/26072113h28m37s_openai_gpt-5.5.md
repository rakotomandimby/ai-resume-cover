...
# OPN I update the form and API prompt flow so special instructions can target both CV and cover letter generation

I implement this feature by changing the special-instructions form from “cover letter only” into a shared instructions section that applies to both generated documents by default. I also add an optional checkbox that reveals separate CV and cover-letter instruction fields. On the backend, I derive the correct instruction text for each document type and pass CV-specific instructions into OpenAI, Gemini, and Anthropic CV generation calls.
  ...
  <body>
      ...
      <form class="mt-4" action="/" method="POST">
        ...
        <fieldset class="mb-3 border rounded p-3">
          ...
          <div class="mb-3 form-check">
            ...
            <label class="form-check-label" for="enableSpecialInstructions">Add custom guidance for generated documents</label>
          </div>

          <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="useSeparateSpecialInstructions" name="useSeparateSpecialInstructions" value="true">
            <label class="form-check-label" for="useSeparateSpecialInstructions">Use separate instructions for the CV and cover letter</label>
          </div>

          <div id="sharedSpecialInstructionsContainer" class="mb-0">
            <label for="specialInstructions" class="form-label">Instructions for both documents</label>
            <textarea id="specialInstructions" class="form-control" rows="3" name="specialInstructions" placeholder="E.g., emphasize my leadership skills, keep the tone enthusiastic, focus on cloud architecture, etc."></textarea>
            <div class="form-text">These instructions are sent to both CV and cover letter generation by default.</div>
          </div>

          <div id="separateSpecialInstructionsContainer" class="row d-none">
            <div class="col-md-6 mb-3 mb-md-0">
              <label for="coverLetterSpecialInstructions" class="form-label">Cover letter instructions</label>
              <textarea id="coverLetterSpecialInstructions" class="form-control" rows="3" name="coverLetterSpecialInstructions" placeholder="E.g., mention motivation for the company, use a concise tone, emphasize team leadership, etc."></textarea>
            </div>
            <div class="col-md-6">
              <label for="cvSpecialInstructions" class="form-label">CV instructions</label>
              <textarea id="cvSpecialInstructions" class="form-control" rows="3" name="cvSpecialInstructions" placeholder="E.g., emphasize backend experience, prioritize cloud skills, keep older roles shorter, etc."></textarea>
            </div>
          </div>
        </fieldset>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        const useSeparateSpecialInstructionsCheckbox = document.getElementById('useSeparateSpecialInstructions');
        const sharedSpecialInstructionsContainer = document.getElementById('sharedSpecialInstructionsContainer');
        const separateSpecialInstructionsContainer = document.getElementById('separateSpecialInstructionsContainer');
        const sharedSpecialInstructionsTextarea = document.getElementById('specialInstructions');
        const coverLetterSpecialInstructionsTextarea = document.getElementById('coverLetterSpecialInstructions');
        ...
        function updateSpecialInstructionsState() {
          ...
          const useSeparate = useSeparateSpecialInstructionsCheckbox.checked;

          useSeparateSpecialInstructionsCheckbox.disabled = !enabled;

          if (!enabled) {
            sharedSpecialInstructionsContainer.classList.remove('d-none');
            separateSpecialInstructionsContainer.classList.add('d-none');

            sharedSpecialInstructionsTextarea.value = '';
            coverLetterSpecialInstructionsTextarea.value = '';
            cvSpecialInstructionsTextarea.value = '';

            sharedSpecialInstructionsTextarea.disabled = true;
            coverLetterSpecialInstructionsTextarea.disabled = true;
            cvSpecialInstructionsTextarea.disabled = true;

            sharedSpecialInstructionsTextarea.placeholder = "Special instructions are disabled";
            return;
          }

          sharedSpecialInstructionsTextarea.placeholder = "E.g., emphasize my leadership skills, keep the tone enthusiastic, focus on cloud architecture, etc.";

          if (useSeparate) {
            sharedSpecialInstructionsContainer.classList.add('d-none');
            separateSpecialInstructionsContainer.classList.remove('d-none');

            sharedSpecialInstructionsTextarea.disabled = true;
            coverLetterSpecialInstructionsTextarea.disabled = false;
            cvSpecialInstructionsTextarea.disabled = false;
          } else {
            sharedSpecialInstructionsContainer.classList.remove('d-none');
            separateSpecialInstructionsContainer.classList.add('d-none');

            sharedSpecialInstructionsTextarea.disabled = false;
            coverLetterSpecialInstructionsTextarea.disabled = true;
            cvSpecialInstructionsTextarea.disabled = true;
          }
        }
        ...
        useSeparateSpecialInstructionsCheckbox.addEventListener('change', updateSpecialInstructionsState);
...
app.post('/', csrfProtection, async (req: Request, res: Response) => {
  ...
  const useSeparateSpecialInstructions = req.body.useSeparateSpecialInstructions === 'true';
  const sharedSpecialInstructions = getStringField(req.body.specialInstructions);
  const submittedCoverLetterSpecialInstructions = getStringField(req.body.coverLetterSpecialInstructions);
  const submittedCVSpecialInstructions = getStringField(req.body.cvSpecialInstructions);

  const coverLetterSpecialInstructions = enableSpecialInstructions
    ? useSeparateSpecialInstructions
      ? submittedCoverLetterSpecialInstructions
      : sharedSpecialInstructions
    : '';

  const cvSpecialInstructions = enableSpecialInstructions
    ? useSeparateSpecialInstructions
      ? submittedCVSpecialInstructions
      : sharedSpecialInstructions
    : '';

  const enableCoverLetterSpecialInstructions = enableSpecialInstructions && coverLetterSpecialInstructions.trim() !== '';
  const enableCVSpecialInstructions = enableSpecialInstructions && cvSpecialInstructions.trim() !== '';
  ...
  const geminiCoverLetterPromise = runGemini
    ? getGeminiCoverLetterResult(
        ...
        words,
        searchCompanyInfo,
        enableCoverLetterSpecialInstructions,
        coverLetterSpecialInstructions,
        dryRun
      )
    : Promise.resolve('Not selected');
...
export function getCoverLetterConversation(
  ...
  enableCoverLetterSpecialInstructions?: boolean,
  coverLetterSpecialInstructions?: string
): ConversationTurn[] {
  if (language === 'French') {
    ...
    if (
      enableCoverLetterSpecialInstructions
      && coverLetterSpecialInstructions
      && coverLetterSpecialInstructions.trim() !== ''
    ) {
      turns.push(
        {
          role: 'assistant',
          content: "Y a-t-il des instructions particulières ou des directives que je devrais suivre lors de la rédaction de la lettre de motivation ?",
        },
        { role: 'user', content: coverLetterSpecialInstructions.trim() }
      );
    }
    turns.push(
    ...
    if (
      ...
      turns.push(
        {
          role: 'assistant',
          content: "Are there any specific instructions or guidelines I should follow while writing the cover letter?",
        },
        { role: 'user', content: coverLetterSpecialInstructions.trim() }
      );
    }
...
// 6. optional assistant/user turn: provide special instructions for the CV
// 7. assistant: ask what to do next (hardcoded)
// 8. user: ask for the tailored CV based on the position and job description
export function getCVConversation(
  language: string,
  cv: string,
  job: string,
  position: string,
  enableCVSpecialInstructions?: boolean,
  cvSpecialInstructions?: string
): ConversationTurn[] {
  if (language === 'French') {
    const turns: ConversationTurn[] = [
    ...
    if (
      enableCVSpecialInstructions
      && cvSpecialInstructions
      && cvSpecialInstructions.trim() !== ''
    ) {
      turns.push(
        {
          role: 'assistant',
          content: "Y a-t-il des instructions particulières ou des directives que je devrais suivre lors de la personnalisation du CV ?",
        },
        { role: 'user', content: cvSpecialInstructions.trim() }
      );
    }
    ...
    if (
      ...
      turns.push(
        {
          role: 'assistant',
          content: "Are there any specific instructions or guidelines I should follow while tailoring the CV?",
        },
        { role: 'user', content: cvSpecialInstructions.trim() }
      );
    }
...
export async function getOpenAICVResult(
  ...
  position: string,
  language: string,
  enableCVSpecialInstructions: boolean = false,
  cvSpecialInstructions: string = '',
  dryRun: boolean = false
): Promise<string> {
  if (dryRun) {
...
I keep the existing cover-letter behavior compatible, but I rename the relevant parameters to make their purpose clearer:
...
I also preserve the default behavior you requested: when the user enters shared special instructions, the backend forwards the same text to both the CV and cover-letter prompt builders. When the user checks “Use separate instructions for the CV and cover letter,” the backend sends each separate textarea only to its corresponding document generation function.

OpenAI gpt-5.5 (13.7k in, 11.5k out)
