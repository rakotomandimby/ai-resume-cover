...
The application calls the Interactions API with:

```text
client.interactions.create({
  model: "gemini-3.1-pro-preview",
  systemInstruction: <generated system instruction>,
  input: prompt
})
```
...
The application expects `interactions.create(...)` to resolve to an Interaction object with at least this structure:
...
The application uses these response properties:
...
- The only Gemini content consumed is the text of the last step's first content item.
...
- If Gemini returns an empty string through the last step's content, the empty string is rendered as the result.
- If the first OpenAI choice is missing, or if Gemini's response object is completely malformed, the corresponding promise rejects and the UI shows an error for that provider/document (though the new implementation defaults to an empty string for missing steps).
